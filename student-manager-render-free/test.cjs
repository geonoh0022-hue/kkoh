...
async function start(){child=spawn(process.execPath,['server.cjs'],{cwd:__dirname,env:{...process.env,PORT:String(port),DATA_DIR:dir,NODE_ENV:'test',ADMIN_PASSWORD:'test-password-123456'}});await new Promise((resolve,reject)=>{child.stdout.once('data',resolve);child.once('exit',code=>reject(Error('Exited '+code)));child.once('error',reject);});}
...
async function login(){const r=await call('/api/login','POST',{username:'admin',password:'test-password-123456'});assert.equal(r.status,200);assert.match(r.headers.get('set-cookie'),/HttpOnly/);return r.headers.get('set-cookie').split(';')[0];}
