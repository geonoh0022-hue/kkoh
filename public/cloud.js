...
(() => {
  ...
  const lock = value => { document.querySelector('main').inert = value; };
  ...
  async function request(path, body, auth = true, method) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) throw Error('로그인이 만료되었습니다. 전체 백업 후 로그아웃하고 다시 로그인하세요.');
  ...
  $('cloudLogin').onsubmit = event => {
    ...
    guarded(async () => {
      ...
      status('로그인됨: '+session.user.email+' · 서버 자료 불러오기 또는 첫 저장을 선택하세요.');
    });
  };
  $('cloudLoad').onclick = () => guarded(async () => {
    ...
    replace(data); revision = row.revision; enabled = true; blocked = false; pending = false; inFlight = null;
  ...
  $('cloudRetry').onclick = () => { if (!enabled || blocked) return status('먼저 서버 자료 불러오기 또는 첫 저장을 선택하세요.'); upload(); };
  $('cloudLogout').onclick = () => guarded(async () => {
    ...
    try { await request('/auth/v1/logout',{},true); } catch {}
    session = null; revision = null; pending = false; inFlight = null; blocked = false;
  ...
  window.addEventListener('storage',event => {
    if (event.key === KEY) {
      clearTimeout(debounce); enabled = false; blocked = true; lock(true);
  ...
  window.addEventListener('online',() => { if (enabled && pending && !blocked) upload(); });
  try { const saved = JSON.parse(localStorage.getItem('gradebook-cloud-config')); if (saved) { $('serverUrl').value = saved.url || ''; $('publicKey').value = saved.key || ''; } } catch {}
  status('브라우저 저장 모드 · 서버 저장은 아래 설정 후 로그인하세요.');
})();
